...
import ConfirmationModal from "@/features/service-category/confirmation-modal";
...
function Toggle({ value, onChange, label }: { value: boolean; onChange: (val: boolean) => void; label?: string }) {
  return (
    <div className="content-stretch flex gap-[12px] items-center relative shrink-0 cursor-pointer" onClick={() => onChange(!value)}>
      <div className={`${value ? 'bg-[#05376d]' : 'bg-[#e5e5e5]'} h-[24px] overflow-clip relative rounded-[100px] shrink-0 w-[45px] transition-colors`}>
        <div className={`absolute bg-white rounded-[100px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.16)] size-[20px] top-[2px] transition-all ${value ? 'left-[23px]' : 'left-[2px]'}`} />
      </div>
      {label && <p className="font-['Inter:Medium',sans-serif] font-medium leading-[20px] not-italic relative shrink-0 text-[#0a0a0a] text-[14px]">{label}</p>}
    </div>
  );
}
...
function InputField({ label, registration, ...props }: InputFieldProps) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start w-full">
      <div className="content-stretch flex gap-[2px] items-start relative shrink-0 w-full">
        <p className="font-['Inter:Medium',sans-serif] font-medium leading-[20px] not-italic relative shrink-0 text-[#171717] text-[14px]">{label}</p>
      </div>
      <div className="bg-white h-[38px] relative rounded-[8px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#e5e5e5] shrink-0 w-full">
        <div className="flex flex-row items-center overflow-clip rounded-[inherit] size-full">
          <input
            {...registration}
            {...props}
            className="content-stretch flex gap-[10px] items-center px-[14px] py-[7px] relative size-full outline-none text-[#171717] text-[14px] font-['Inter:Regular',sans-serif] placeholder:text-[#737373] bg-transparent"
          />
        </div>
      </div>
    </div>
...
function SelectField({ label, options, registration, ...props }: SelectFieldProps) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start w-full">
      ...
      <div className="bg-white h-[38px] relative rounded-[8px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#e5e5e5] shrink-0 w-full">
        <div className="flex flex-row items-center overflow-clip rounded-[inherit] size-full px-[14px] relative">
          <select
            {...registration}
            {...props}
            className="w-full h-full outline-none text-[#171717] text-[14px] font-['Inter:Regular',sans-serif] bg-transparent appearance-none z-10 relative cursor-pointer"
          >
          ...
          <div className="pointer-events-none absolute right-[14px] top-1/2 -translate-y-1/2 flex items-center z-0">
...
type TextAreaFieldProps = {
  ...
  isHtml?: boolean;
  registration?: UseFormRegisterReturn;
} & Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, keyof UseFormRegisterReturn>;

function TextAreaField({ label, isHtml, registration, ...props }: TextAreaFieldProps) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start w-full">
      <div className="content-stretch flex gap-[2px] items-start relative shrink-0 w-full">
        <p className="font-['Inter:Medium',sans-serif] font-medium leading-[20px] not-italic relative shrink-0 text-[#171717] text-[14px]">{label}</p>
      </div>
      <div className="bg-white content-stretch flex flex-col h-[240px] items-start overflow-clip relative rounded-[8px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#e5e5e5] shrink-0 w-full">
        {isHtml && (
           <div className="h-[50px] relative shrink-0 w-full border-b border-[#e5e5e5] flex items-center px-[16px] gap-[20px] overflow-x-auto scrollbar-hide">
              {/* Mock Toolbar Icons */}
              {[svgPaths.p331f9200, svgPaths.p268a3f00, svgPaths.p2ec9c400, svgPaths.p5e7b300, svgPaths.p13a2d900, svgPaths.p2a696580, svgPaths.p22b4a1c0, svgPaths.p2bd4e980].map((path, i) => (
                  <div key={i} className="size-[18px] relative shrink-0 cursor-pointer opacity-70 hover:opacity-100">
                      <svg className="size-full" fill="none" viewBox="0 0 18 18">
                          <path d={path} fill="#404040" fillRule="evenodd" clipRule="evenodd" />
                      </svg>
                  </div>
              ))}
           </div>
        )}
        <div className="flex-[1_0_0] relative w-full p-[14px]">
           <textarea
             {...registration}
             {...props}
             className="w-full h-full resize-none outline-none text-[#262626] text-[16px] font-['DM_Mono:Regular',sans-serif] bg-transparent"
           />
        </div>
        <div className="absolute bottom-[4px] right-[4px] size-[12px]">
            <svg className="block size-full" fill="none" viewBox="0 0 12.0754 12.0754">
                <path d={svgPaths.p1df11c0} stroke="#D4D4D4" />
            </svg>
        </div>
      </div>
    </div>
...
export default function ServiceCategoryEditPage() {
  ...
  const { register, handleSubmit, control, watch, reset } = useForm({
    defaultValues: {
      ...
      vatCategory: '',
      freeRentalDays: '',
      multipleInventory: false,
      serviceFrequency: false,
      clientImageUploading: false,
      disableEmailSending: false,
      availableForPromo: true,
      promoCategories: '',
      
      titleEn: '', subtitleEn: '', confirmTextEn: '', priceInfoEn: '', bodyEn: '',
      titleLv: '', subtitleLv: '', confirmTextLv: '', priceInfoLv: '', bodyLv: '',
      titleRu: '', subtitleRu: '', confirmTextRu: '', priceInfoRu: '', bodyRu: '',
    }
  });
  ...
  useEffect(() => {
    if (isEditMode) {
      reset({
        ...
        mobileName: 'Lorem Ipsum',
        description: 'King put on your head-- Do you think you might do something better with the Lory, with a round face, and large eyes full of soup. \'There\'s certainly too much overcome to do anything but sit with its.',
        htmlDescription: 'King put on your head-- Do you think you might do something better with the Lory, with a round face, and large eyes full of soup. \'There\'s certainly too much overcome to do anything but sit with its.',
        serviceType: '1',
        postPaymentOnly: false,
        publicView: true,
        ...
        availableForPromo: true,
        promoCategories: '1',
        titleEn: 'Containers for wood waste', subtitleEn: 'Lorem Ipsum', confirmTextEn: '', priceInfoEn: '', bodyEn: '',
        titleLv: 'Containers for wood waste', subtitleLv: 'Lorem Ipsum', confirmTextLv: '', priceInfoLv: '', bodyLv: '',
        titleRu: 'Containers for wood waste', subtitleRu: 'Lorem Ipsum', confirmTextRu: '', priceInfoRu: '', bodyRu: '',
      });
    } else {
       reset({
          fullName: '',
  ...
  const handleConfirmUpdate = () => {
    // Logic to save update with date
    console.log('Updating service category with date:', effectiveDate);
    setShowModal(false);
    navigate('/services');
  };
       ...
       <div className="w-full max-w-[1220px] flex flex-col gap-[32px] pt-[32px]">
          ...
          <div className="flex flex-col gap-[14px] w-full">
             <div className="flex items-center gap-[14px]">
                <div className="bg-white px-[14px] py-[4px] rounded-[8px] border border-transparent">
                   <span className="text-[#737373] text-[14px] font-medium cursor-pointer" onClick={() => navigate('/services')}>Services</span>
                </div>
                <div className="flex items-center gap-[14px]">
                   <svg className="size-[10px] -rotate-90 text-[#A3A3A3]" fill="none" viewBox="0 0 10 6">
                      <path clipRule="evenodd" d={svgPaths.p2ae3a500} fill="currentColor" fillRule="evenodd" />
                   </svg>
                   <span className="text-[#737373] text-[14px] font-medium">{isEditMode ? 'Edit service category' : 'New service category'}</span>
                </div>
             </div>
             
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4 sm:gap-0">
                <h1 className="text-[#171717] text-[24px] sm:text-[30px] font-bold leading-[32px]">{isEditMode ? 'Edit service category' : 'New service category'}</h1>
                <div className="flex items-center gap-[12px]">
                    <Controller
                      name="publicView"
                      control={control}
                      render={({ field }) => (
                        <Toggle value={field.value} onChange={field.onChange} label="Public view" />
                      )}
                    />
                </div>
             </div>
          ...
          {/* Card 1: General Info */}
          <div className="bg-white rounded-[12px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#ececee] p-[24px] flex flex-col gap-[24px]">
             <div className="flex flex-col lg:flex-row gap-[24px] items-start w-full">
                <InputField label="Full name" registration={register('fullName')} />
                <InputField label="Short name" registration={register('shortName')} />
                <InputField label="Mobile name" registration={register('mobileName')} />
             </div>
             
             <div className="flex flex-col lg:flex-row gap-[24px] items-start w-full">
                <TextAreaField label="Description" registration={register('description')} isHtml />
                <TextAreaField label="HTML-Description" registration={register('htmlDescription')} isHtml />
             </div>
             
             <div className="flex flex-col lg:flex-row gap-[24px] items-start w-full">
                <div className="flex flex-col gap-[24px] w-full">
                   <div className="flex flex-col gap-[8px]">
                      <SelectField label="Service type" options={[{value: '1', label: 'Type 1'}, {value: '2', label: 'Type 2'}]} registration={register('serviceType')} />
                   </div>
                   <Controller
                      name="postPaymentOnly"
                      control={control}
                      render={({ field }) => (
                        <Toggle value={field.value} onChange={field.onChange} label="Postpayment only" />
                      )}
                   />
                </div>
                
                <div className="flex flex-col gap-[8px] w-full lg:w-[574px] shrink-0">
                    <p className="font-['Inter:Medium',sans-serif] font-medium leading-[20px] text-[#171717] text-[14px]">Photo for Web and App</p>
                    <div className="bg-white border border-[#e5e5e5] rounded-[8px] shadow-sm py-[29px] flex items-center justify-center cursor-pointer hover:bg-gray-50 relative overflow-hidden">
                        <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" />
                        <p className="text-[#737373] text-[14px]">Drag & Drop your files or <span className="text-[#05376d]">Browse</span></p>
                    </div>
                    <p className="text-[#737373] text-[14px]">Optimal image dimensions: 180x180px</p>
                </div>
             </div>
          </div>
          ...
          <div className="bg-white rounded-[12px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#ececee] overflow-hidden">
             <div className="px-[24px] py-[16px] border-b border-[#e5e5e5]">
                 <h2 className="text-[#171717] text-[18px] font-semibold">Configurations</h2>
             </div>
             <div className="p-[24px] flex flex-col gap-[24px]">
                 <div className="flex flex-col lg:flex-row gap-[24px] items-end w-full">
                     <SelectField label="Inventory categories" options={[{value: '1', label: 'Cat 1'}]} registration={register('inventoryCategories')} />
                     <SelectField label="VAT Category" options={[{value: '21', label: '21%'}]} registration={register('vatCategory')} />
                     <div className="w-full flex flex-col gap-[8px]">
                         <SelectField label="Free rental days" options={[{value: '3', label: '3 days'}]} registration={register('freeRentalDays')} />
                         <p className="text-[#737373] text-[14px]">Days included in the price before the paid period</p>
                     </div>
                 </div>
                 
                 <div className="flex flex-col lg:flex-row gap-[24px] items-start lg:items-center">
                    <Controller name="multipleInventory" control={control} render={({field}) => <Toggle value={field.value} onChange={field.onChange} label="Multiple Inventory in 1 order" />} />
                    <Controller name="serviceFrequency" control={control} render={({field}) => <Toggle value={field.value} onChange={field.onChange} label="Service frequency" />} />
                    <Controller name="clientImageUploading" control={control} render={({field}) => <Toggle value={field.value} onChange={field.onChange} label="Client image uploading" />} />
                 </div>
                 
                 <div>
                    <Controller name="disableEmailSending" control={control} render={({field}) => <Toggle value={field.value} onChange={field.onChange} label="Disabling email sending for free services" />} />
                 </div>
                 
                 <div className="flex flex-col gap-[24px] items-start w-full">
                     <Controller name="availableForPromo" control={control} render={({field}) => <Toggle value={field.value} onChange={field.onChange} label="Available for promo" />} />
                     <div className="w-full lg:w-[574px]">
                         <SelectField label="Promo categories" options={[{value: '1', label: 'Promo 1'}]} registration={register('promoCategories')} />
                     </div>
                 </div>
             </div>
          </div>
          ...
          <div className="bg-white rounded-[12px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.06),0px_0px_0px_1px_#ececee] overflow-hidden">
             <div className="px-[24px] py-[16px] border-b border-[#e5e5e5]">
                 <h2 className="text-[#171717] text-[18px] font-semibold">Service terms</h2>
             </div>
             <div className="p-[24px] flex flex-col gap-[24px]">
                 {/* EN Section */}
                 <div className="bg-[#fafafa] rounded-[12px] shadow-sm p-[24px] border border-[#ececee] flex flex-col gap-[24px]">
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <InputField label="Title (EN)" registration={register('titleEn')} />
                         <InputField label="Subtitle (EN)" registration={register('subtitleEn')} />
                     </div>
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <TextAreaField label="Confirmation text (EN)" registration={register('confirmTextEn')} isHtml />
                         <TextAreaField label="Price information (EN)" registration={register('priceInfoEn')} isHtml />
                         <TextAreaField label="HTML-Text (Body) (EN)" registration={register('bodyEn')} isHtml />
                     </div>
                 </div>
                 ...
                 <div className="bg-[#fafafa] rounded-[12px] shadow-sm p-[24px] border border-[#ececee] flex flex-col gap-[24px]">
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <InputField label="Title (LV)" registration={register('titleLv')} />
                         <InputField label="Subtitle (LV)" registration={register('subtitleLv')} />
                     </div>
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <TextAreaField label="Confirmation text (LV)" registration={register('confirmTextLv')} isHtml />
                         <TextAreaField label="Price information (LV)" registration={register('priceInfoLv')} isHtml />
                         <TextAreaField label="HTML-Text (Body) (LV)" registration={register('bodyLv')} isHtml />
                     </div>
                 </div>
                 ...
                 <div className="bg-[#fafafa] rounded-[12px] shadow-sm p-[24px] border border-[#ececee] flex flex-col gap-[24px]">
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <InputField label="Title (RU)" registration={register('titleRu')} />
                         <InputField label="Subtitle (RU)" registration={register('subtitleRu')} />
                     </div>
                     <div className="flex flex-col lg:flex-row gap-[24px]">
                         <TextAreaField label="Confirmation text (RU)" registration={register('confirmTextRu')} isHtml />
                         <TextAreaField label="Price information (RU)" registration={register('priceInfoRu')} isHtml />
                         <TextAreaField label="HTML-Text (Body) (RU)" registration={register('bodyRu')} isHtml />
                     </div>
                 </div>
             </div>
          </div>
